...
async function getAllUsers() {
  try {
    ...
    res.data.forEach((user) => {
      ...
      li.className =
        "list-group-item list-group-item-action  w-75 m-2";
      li.appendChild(
        document.createTextNode(
          `${user.description} ${user.catagory} ${user.amount}`
        )
      );
...
async function deleteUser(e) {
  try {
    if (e.target.classList.contains("delete")) {
      ...
      const catagory = userdata[userdata.length - 2];
      const amount = userdata[userdata.length - 1];
      const userInfo = { catagory, amount };
      const res = await axios.get(`http://localhost:3000/get/users`);
      res.data.forEach((user) => {
        console.log(userdata);
        if (user.catagory == catagory && user.amount == amount) {
          console.log("enters");
          console.log(user.id);
          id = user.id;
        }
